...
// при удалении пользователя, все его объявления удаляются каскадом
// здесь onDelete('restrict') - по умолчанию. нельзя удалить категорию, в которой есть объявления
...
class CreateAdvertsTables extends Migration
{
    ...
    public function up()
    {
        Schema::create('advert_adverts', function(Blueprint $table){
           ...
           $table->integer('category_id')->references('id')->on('advert_category');
